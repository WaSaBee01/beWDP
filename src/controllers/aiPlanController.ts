...
export const generateDailyPlan = async (req: AuthRequest, res: Response): Promise<void | Response> => {
  try {
    if (!req.user) {
...
Hãy tạo một kế hoạch dinh dưỡng và luyện tập cho 1 ngày với cấu trúc JSON sau:
...
Lưu ý:
- Tạo ít nhất 3 bữa ăn (sáng, trưa, tối) và có thể thêm bữa phụ
...
- Tạo ít nhất 1 bài tập phù hợp với mức độ và mục tiêu
- Đảm bảo tổng calo nạp vào phù hợp với mục tiêu (khoảng ${survey.dailyCalories || 2000} cal)
