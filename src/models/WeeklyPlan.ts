import mongoose, { Document, Schema, Types } from 'mongoose';

export interface IWeeklyPlan extends Document {
  name: string;
  description?: string;
  goal?: 'weight_loss' | 'muscle_gain' | 'healthy_lifestyle';
  isCommon: boolean;
  createdBy: Types.ObjectId;
  days: {
    monday?: Types.ObjectId;
    tuesday?: Types.ObjectId;
    wednesday?: Types.ObjectId;
    thursday?: Types.ObjectId;
    friday?: Types.ObjectId;
    saturday?: Types.ObjectId;
    sunday?: Types.ObjectId;
  };
  totals: {
    caloriesIn: number;
    carbs: number;
    protein: number;
    fat: number;
    caloriesOut: number;
  };
  createdAt: Date;
  updatedAt: Date;
}

const weeklyPlanSchema = new Schema<IWeeklyPlan>(
  {
    name: { type: String, required: true, trim: true },
    description: { type: String, default: '' },
    goal: { type: String, enum: ['weight_loss', 'muscle_gain', 'healthy_lifestyle'], default: 'healthy_lifestyle' },
    isCommon: { type: Boolean, default: false },
    createdBy: { type: Schema.Types.ObjectId, ref: 'User', required: true },
    days: {
      monday: { type: Schema.Types.ObjectId, ref: 'Plan' },
      tuesday: { type: Schema.Types.ObjectId, ref: 'Plan' },
      wednesday: { type: Schema.Types.ObjectId, ref: 'Plan' },
      thursday: { type: Schema.Types.ObjectId, ref: 'Plan' },
      friday: { type: Schema.Types.ObjectId, ref: 'Plan' },
      saturday: { type: Schema.Types.ObjectId, ref: 'Plan' },
      sunday: { type: Schema.Types.ObjectId, ref: 'Plan' },
    },
    totals: {
      caloriesIn: { type: Number, default: 0 },
      carbs: { type: Number, default: 0 },
      protein: { type: Number, default: 0 },
      fat: { type: Number, default: 0 },
      caloriesOut: { type: Number, default: 0 },
    },
  },
  { timestamps: true }
);

weeklyPlanSchema.index({ name: 'text', description: 'text' });

export const WeeklyPlan = mongoose.model<IWeeklyPlan>('WeeklyPlan', weeklyPlanSchema);


